...
public class CreateSinglePageQuiz {

    public String generateUI(QuestionType questionType, Question question, boolean isEditable) {
        switch (questionType) {
            case QUESTION_RESPONSE:
                return generateQuesRes(question) + generateCorrectResponse(question) + generateEditButtons(question, isEditable);
            case FILL_IN_THE_BLANK:
                return generateFillBlank(question) + generateCorrectResponse(question) + generateEditButtons(question, isEditable);
            case MULTIPLE_CHOICE:
                return generateMultiChoice(question, isEditable);
            case PICTURE_RESPONSE:
                return generatePictRes(question) + generateCorrectResponse(question) + generateEditButtons(question, isEditable);
            case MULTI_ANSWER:
    ...
    private String generateMultiChoice(Question question, boolean isEditable) {
        ...
        if (isEditable) {
            html.append(generateEditButtons(question, isEditable));
        }
    ...
    private String generateEditButtons(Question question, boolean isEditable) {
    ...
    private String generateCorrectResponse(Question question) {
        StringBuilder html = new StringBuilder();
        ...
        switch (question.getQuestionType()) {
            ...
            case FILL_IN_THE_BLANK:
            case PICTURE_RESPONSE:
                html.append(question.getSingleQuestionAnswer());
                break;
            case MULTIPLE_CHOICE:
            case MULTI_ANSWER:
            case MULTIPLE_CHOICE_WITH_ANSWERS:
                html.append(String.join(", ", question.getMultipleChoiceAnswers()));
                break;
            case MATCHING:
                html.append(question.getMatchingPairs().toString());
                break;
            default:
                html.append("No correct answer available.");
        }
        html.append("</div>");
        return html.toString();
    }
